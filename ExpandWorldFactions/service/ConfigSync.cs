...
using ExpandWorldData.Factions;
...
public class ConfigSync
{
  ...
  {
    public readonly Dictionary<OwnConfigEntryBase, object?> configValues = [];
    public readonly Dictionary<CustomSyncedValueBase, object?> customValues = [];
  }
...
public class VersionCheck
{
  ...
  private static void CheckVersion(ZRpc rpc, ZPackage pkg, Action<ZRpc, ZPackage>? original)
  {
    ...
    if (matched)
    {
      ...
      {
        Dictionary<Character.Faction, string> factions = [];
        while (pkg.m_reader.BaseStream.Position != pkg.m_reader.BaseStream.Length)
        {
          var faction = (Character.Faction)pkg.ReadInt();
          var name = pkg.ReadString();
          factions.Add(faction, name);
        }
        FactionManager.SetNames(factions);
      }
    }
    if (!matched)
    {
  ...
  private static void RegisterAndCheckVersion(ZNetPeer peer, ZNet __instance)
  {
    ...
    foreach (VersionCheck check in versionChecks)
    {
      ...
      if (ZNet.instance.IsServer())
      {
        foreach (var kvp in FactionManager.FactionToDisplayName)
        {
